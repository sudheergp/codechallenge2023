...
# temporary_credentials = oauthlib.oauth1.rfc5849.credentials.Credentials.from_string(response.text)
#
# oauthlib.oauth1.rfc5849
# # Ask the user to authorize the temporary token
# authorize_url = "https://api.twitter.com/oauth/authorize?oauth_token=" + temporary_credentials["oauth_token"]
# print("Please visit this URL to authorize the app:", authorize_url)
# oauth_verifier = input("Enter the OAuth verifier: ")
#
...
# client = oauthlib.oauth1.Client(consumer_key, client_secret=consumer_secret,
#                                resource_owner_key=temporary_credentials["oauth_token"],
#                                resource_owner_secret=temporary_credentials["oauth_token_secret"])
# uri, headers, body = client.sign(token_endpoint, verifier=oauth_verifier)
# response = requests.post(uri, headers=headers, data=body)
# access_credentials = oauthlib.oauth1.rfc5849.credentials.Credentials.from_string(response.text)
#
# # Print the access token and secret (these are your write access credentials)
# print("Access token:", access_credentials["oauth_token"])
# print("Access token secret:", access_credentials["oauth_token_secret"])
